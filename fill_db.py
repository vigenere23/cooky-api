...
def generateUser():
        userDao = UserDao()
        for i in range(100):
                tmp = UserModel(id=None, username=listeMot[i], bio=loremBio)
                userDao.save(tmp)
...
def generateAccount():
        ...
        for i in range(100):
                lastName = choice(listeMot)
                firstName = choice(listeMot)
                accountDao.save(AccountModel(id=None, id_User=(randint(1, 100)), id_Address=(randint(1, 100)), firstName=firstName, lastName=lastName, email=firstName + "." + lastName + "@gmail.com", password=choice(listeMot)+choice(listeMot)+choice(listeMot)))

def generateCommand():
        commandDao = CommandsDao()
        for i in range(100):
                current_time = datetime.datetime.now() 
                commandDao.save(CommandsModel(id=None, id_Cart=randint(1, 100), creationDate=current_time, arrivalDate=current_time))
